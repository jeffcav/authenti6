...
#define AUTHOR_ORGANIZATION "Instituto Atlantico"
 ...
 */
inline double diff_time(struct timespec begin, struct timespec end) {
	...
	while (iats_remaining && consecutive_timeouts < MAX_NUM_TIMEOUTS) {
		...
		while (num_reports--) {
			...
			if (tstart.tv_sec != 0 || tstart.tv_nsec != 0) {
				if (iats_remaining) {
					iats[iat_idx++] = diff_time(tstart, tend);
					iats_remaining--;
				}
			}
